'Numerical check for Exercise 7.3.'
...
def planner_or_monopoly(monopoly: bool) -> QuadResult:
    a0, a1, beta, d = (100.0, 0.05, 0.95, 10.0)
    a = np.array([[1.0, 0.0], [0.0, 0.0]])
    b = np.array([[0.0], [1.0]])
    r = np.zeros((2, 2))
    r[0, 1] = r[1, 0] = a0 / 2.0
    r[1, 1] = (-a1 if monopoly else -0.5 * a1) - 0.5 * d
    q = np.array([[-0.5 * d]])
    h = np.array([[0.0, d / 2.0]])
